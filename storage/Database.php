...
abstract class databaseAdapter {
    ...
    protected function getConnector($key = null)
    {
        ...
        if(!$connector["name"]) {
            Error::register(self::TYPE . "_database_connection_failed", "database");
            return false;
        }
    ...
    private function getQuery($action, $table_name = null)
	{
        ...
        if($this->loadDriver()) {
            ...
            if(!$this->key_name)                                    { Error::register(self::TYPE . " key missing", "database"); }
            if(!$this->getTable("name"))                        { Error::register(self::TYPE . " table missing", "database"); }
            ...
            Error::register("Connection failed to database: " . self::TYPE, "database");
        }
        return $query;
    }
                ...
                if($db) {
                    ...
                    if($db["num_rows"] < $this::MAX_NUMROWS) {
                        if($this->rawdata || $db["num_rows"] > $this::MAX_RESULTS) {
                            ...
                            if($db["count"])                        { $res["count"] = $db["count"]; }
                        }
...
class Database {
    ...
    public static function getInstance($databaseAdapters, $params = null) {
        ...
        if(!self::$singletons[$key])                                        { self::$singletons[$key] = new Database($databaseAdapters, $params); }
    ...
    public function __construct($databaseAdapters = self::ADAPTER, $params = null) {
        if(is_array($databaseAdapters)) {
            ...
            $class_name                                                     = self::TYPE . ucfirst($databaseAdapters);
